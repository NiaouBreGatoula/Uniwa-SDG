    ...
    (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      setAppState((prev) => {
        ...
        if (!desiredIndicator) {
          ...
          if (desiredIndicator.type === "white") {
              desiredIndicator.currentValue = Number(inputValue); 
              const updatedMap = new Map(stateClone);
              updatedMap.set(inputName, desiredIndicator);
              return updatedMap;
          }


          if (desiredIndicator.type !== "pink" && desiredIndicator.type !== "green")
          {
              console.error(
